...
import com.qiqiim.common.utils.BlankUtil;
...
//        	RedisUtil redisUtil = new RedisUtil();
            ...
            String s = (String) redisUtil.get("SESSIONID:" + session.getAccount());
            if(StringUtils.isNotBlank(s)) {
                redisUtil.set("NID:" + session.getNid(), s);
            }
        }
        //全员发送上线消息
        ImMessage model = proxy.getOnLineStateMsg(session.getAccount());
    ...
    public synchronized void removeSession(String sessionId) {
    	try{
    		...
    		if(session!=null){
                ...
                ImMessage model = proxy.getOffLineStateMsg(sessionId);
				ImChannelGroup.broadcast(model);
//				DwrUtil.sedMessageToAll(model);
    		}  
    	}catch(Exception e){
...
//    				DwrUtil.sedMessageToAll(model);
     ...
     */
    @Override
    public int addSessionGroupId(MessageWrapper wrapper, ChannelHandlerContext ctx){
    	String sessionId = wrapper.getSessionId();
    	Session session = sessions.get(sessionId);
        ImMessage model = (ImMessage)wrapper.getBody();
    	//顺便查询房间存不存在********************************************************************未写完
    	if (session != null && !BlankUtil.isBlank(model.getGroupId())) {
    		session.setUpdateTime(new Date().getTime());
    		redisUtil.hset(Constants.GroupId.ROOM_ID+model.getGroupId(), sessionId, StringMyUtils.ObjToJson(session), 60);	
    	}
		return 0;
    }
...
//	}
    ...
    private Session  setSessionContent(ChannelHandlerContext ctx,MessageWrapper wrapper,String sessionId){
    	 log.info("create new session " + sessionId + ", ctx -> " + ctx.toString());
    	  ImMessage message = (ImMessage) wrapper.getBody();
//    	  MessageProto.Model model = (MessageProto.Model)wrapper.getBody();
          ...
          session.setAppKey(message.getAppKey());
          session.setDeviceId(message.getDeviceId());
          session.setPlatform(message.getPlatform());
          session.setPlatformVersion(message.getPlatformVersion());
          session.setSign(message.getSign());
          session.setBindTime(System.currentTimeMillis());
          session.setUpdateTime(session.getBindTime());
          log.info("create new session " + session.toString() + " successful!");
          return session;
    }
